<template>
  ...
  <v-container>
    ...
    >
      <needed-item
        v-for="(neededItem, itemIndex) in paginatedTaskItems"
        :key="'task-' + itemIndex"
        :need="neededItem"
    ...
    >
      <needed-item
        v-for="(neededItem, itemIndex) in paginatedHideoutItems"
        :key="'hideout-' + itemIndex"
        :need="neededItem"
        :item-style="neededItemsStyle"
      />
    </v-row>
    <!-- Load More Button -->
    <v-row v-if="hasMoreItems" justify="center" class="mt-4">
      <v-col cols="auto">
        <v-btn
          color="primary"
          variant="outlined"
          ...
          :loading="loadingMore"
        >
          Load More ({{ displayedCount }} / {{ totalCount }})
        </v-btn>
      </v-col>
    </v-row>
  </v-container>
...
<script setup>
  ...
  const paginatedTaskItems = computed(() => {
    ...
    return neededTaskItems.value.slice(0, limit);
  });

  const paginatedHideoutItems = computed(() => {
    const limit = currentPage.value * ITEMS_PER_PAGE;
    return neededHideoutItems.value.slice(0, limit);
  });

  const totalCount = computed(() => {
    if (activeNeededView.value === 'tasks') return neededTaskItems.value.length;
    if (activeNeededView.value === 'hideout') return neededHideoutItems.value.length;
    return neededTaskItems.value.length + neededHideoutItems.value.length;
  });

  const displayedCount = computed(() => {
    if (activeNeededView.value === 'tasks') return paginatedTaskItems.value.length;
    if (activeNeededView.value === 'hideout') return paginatedHideoutItems.value.length;
    return paginatedTaskItems.value.length + paginatedHideoutItems.value.length;
  });

  const hasMoreItems = computed(() => displayedCount.value < totalCount.value);
