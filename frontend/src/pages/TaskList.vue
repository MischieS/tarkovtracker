<template>
  ...
  <v-container>
                  ...
                  <v-container class="ma-0 pa-0">
                    <v-row dense>
                      <v-col cols="12">
                        ...
                        ></v-switch>
                        <v-switch
                          v-model="showOnlyLightkeeperTasks"
                          label="Lightkeeper Tasks Only"
                          inset
                          true-icon="mdi-eye"
                          false-icon="mdi-eye-off"
                          :color="showOnlyLightkeeperTasks ? 'success' : 'grey'"
                          hide-details
                          density="compact"
                        ></v-switch>
                        <v-switch
                          v-model="showOnlyKappaTasks"
                          label="Kappa Required Only"
                          inset
                          true-icon="mdi-trophy"
                          false-icon="mdi-trophy-outline"
                          :color="showOnlyKappaTasks ? 'warning' : 'grey'"
                          hide-details
                          density="compact"
                        ></v-switch>
                      </v-col>
    ...
    <v-row v-show="!loadingTasks && !reloadingTasks" justify="center">
      ...
      <!-- Load More Button -->
      <v-col v-if="hasMoreTasks" cols="12" class="text-center my-2">
        <v-btn
          color="primary"
          variant="outlined"
          @click="loadMoreTasks"
        >
          Load More ({{ paginatedTasks.length }} / {{ visibleTasks?.length || 0 }})
        </v-btn>
      </v-col>
    </v-row>
  </v-container>
...
<script setup>
  ...
  // Additional filters
  const showOnlyLightkeeperTasks = ref(false);
  const showOnlyKappaTasks = ref(false);
  ...
  const paginatedTasks = computed(() => {
    if (!visibleTasks.value) return [];
    return visibleTasks.value.slice(0, taskPage.value * TASKS_PER_PAGE);
  });
  
  const hasMoreTasks = computed(() => {
    return paginatedTasks.value.length < (visibleTasks.value?.length || 0);
  });
  ...
  watch([activePrimaryView, activeSecondaryView, activeUserView, activeMapView, activeTraderView], () => {
    taskPage.value = 1;
  });
  ...
  const updateVisibleTasks = async function () {
    ...
    try {
      ...
      visibleTaskList = visibleTaskList.filter((task) => {
        ...
        // Check hide non-Kappa filter
        if (hideNonKappaTasks?.value && task.kappaRequired !== true) return false;
        
        // Check Kappa-only filter
        if (showOnlyKappaTasks.value && task.kappaRequired !== true) return false;
        
        // Check Lightkeeper filter
        if (showOnlyLightkeeperTasks.value && task.lightkeeperRequired !== true) return false;
