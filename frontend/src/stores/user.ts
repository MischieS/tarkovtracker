...
// Load user settings from localStorage on store creation
const USER_STORAGE_KEY = 'tarkovtracker_user_settings';

function loadUserSettings(): Partial<UserState> {
  try {
...
let userSaveTimeout: ReturnType<typeof setTimeout> | null = null;
watch(
  () => {
    try {
  ...
  (state) => {
    if (state) {
      ...
      userSaveTimeout = setTimeout(() => {
        const toSave = { ...state };
        delete toSave.saving;
        localStorage.setItem(USER_STORAGE_KEY, JSON.stringify(toSave));
      }, 300);
    }
  },
  { deep: true }
