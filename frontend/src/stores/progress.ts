...
/** Extract current game mode progress data from a store with legacy fallback */
function getProgressData(store: Store<string, UserState> | undefined): UserProgressData | undefined {
  if (!store?.$state) return undefined;
  const gameMode = store.$state.currentGameMode || 'pvp';
  return store.$state[gameMode] || store.$state;
}
export const useProgressStore = defineStore(
  ...
  () => {
    ...
    const tasksCompletions = computed(() => {
      ...
      for (const task of tasks.value as Task[]) {
        ...
        for (const teamId of Object.keys(visibleTeamStores.value)) {
          const currentData = getProgressData(visibleTeamStores.value[teamId]);
    ...
    const hideoutLevels = computed(() => {
      ...
      for (const station of hideoutStations.value) {
        ...
        for (const teamId of Object.keys(visibleTeamStores.value)) {
          ...
          const currentData = getProgressData(store);
    ...
    const getTeamIndex = (teamId: string): string => {
      return teamId === currentUser?.id ? 'self' : teamId;
    };
    const getDisplayName = (teamId: string): string => {
      const storeKey = getTeamIndex(teamId);
      const currentData = getProgressData(teamStores.value[storeKey]);
    ...
    const hasCompletedTask = (teamId: string, taskId: string): boolean => {
      const storeKey = getTeamIndex(teamId);
      const currentData = getProgressData(teamStores.value[storeKey]);
      return currentData?.taskCompletions?.[taskId]?.complete === true;
    };
